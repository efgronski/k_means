...
def euclidean_distance(point1, point2):
    ...
    for x in point1:
        ...
        euc_value = (x - y)**2
        pos = pos + 1
        euclidean = euclidean + euc_value
        euc_value = 0
    euclidean_distance = math.sqrt(euclidean)
...
def update_assignment(list_of_points, centroids_dict):
    ...
    for x in list_of_points:
        ...
        if str(closest_centroid) in assignment_dict:
            assignment_dict[str(closest_centroid)].append(x)
        else:
...
def mean_of_points(list_of_points):
    ...
    mean = []
    sum = 0
    columns = len(list_of_points[0])
    for j in range(0, columns):
        rows = len(list_of_points)
        for i in range(0, rows):
            num = list_of_points[i][j]
            sum = sum + num
        mean_num = sum/rows
...
def update_centroids(assignment_dict):
    ...
    for i in assignment_dict:
        value = assignment_dict[i]
        mean_value = mean_of_points(value)
        updated_cent[i] = mean_value
    return updated_cent
